...
die "ANT_HOME environment variable is not set" if ( not $ENV{ANT_HOME} );
my $ANT = "ant";
...
my $NTprojdir           = catfile( $working_dir, "" );

my $studentTests = $cfg->getProperty( 'studentTests', 0 );
...
my $antLog              = catfile( $log_dir, $antLogRelative );
my $scriptLogRelative   = "script.log";
my $scriptLog           = catfile( $log_dir, $scriptLogRelative );
my $compileLogRelative  = "compile.log";
my $compileLog          = catfile( $log_dir, $compileLogRelative );
my $testLogRelative     = "test.log";
my $testLog             = catfile( $log_dir, $testLogRelative );
my $instrLogRelative    = "instr.log";
my $instrLog            = catfile( $log_dir, $instrLogRelative );
my $timeoutLogRelative  = "timeout.log";
my $timeoutLog          = catfile( $log_dir, $timeoutLogRelative );
my $markupPropFile      = catfile( $pluginHome, "markup.properties" );
my $explanationRelative = "explanation.log";
my $explanation         = catfile( $log_dir, $explanationRelative );
my $covfile             = catfile( $log_dir, "test.cov" );
my $can_proceed         = 1;
my $runtimeScore        = 0;
my $staticScore         = 0;
...
my $onlyInstructorTestsAllowed = $studentTests eq 0;
my $allStudentTestsMustPass    = $studentTests eq 2;

my $coverageMetric = $cfg->getProperty( 'coverageMetric', 0 );
my $measureCodeCoverage = $coverageMetric > 0;
$cfg->setProperty( 'disableCodeCoverage', 1 ) if not $measureCodeCoverage;
...
$ENV{'PATH'} = catfile( $bullseyeDir, "bin", $ENV{'PATH'} ) if $bullseyeDir;

#=============================================================================
# Generate derived properties for ANT
#=============================================================================
#my @scriptDataDirs = < $pluginData/* >;
$pluginData =~ s,/$,,;
...
# testCases
my $testCasePath = catfile( $pluginHome, "tests" );
{
	my $testCaseFileOrDir = $cfg->getProperty('testCases');
	if ( defined $testCaseFileOrDir and $testCaseFileOrDir ne "" ) {
...
sub prep_for_output {
	...
	# print "\t1: $result";
	$result =~ s,([a-z]:)?\Q$pluginHome\E(/[^\s]*(:[0-9]+)?:)?,,gi;
	$result =~ s,([a-z]:)?\Q$DOSStyle_script_home\E(\\[^\s]*(:[0-9]+)?:)?,,gi;

	# print "\t2: $result";
	$result =~ s,([a-z]:)?\Q$pluginHome\E/tests(/[^\s]*(:[0-9]+)?:)?,,gi;
	$result =~
	...
	$result =~
	  s,([a-z]:)?\Q$testCasePath\E(/)?[^:\s]*\.h,<<reference tests>>,gi;
	$result =~ s,([a-z]:)?\Q$testCasePath\E(/)?,,gi;
	$result =~
	  s,([a-z]:)?\Q$unixTestCasePath\E(/)?[^:\s]*\.h,<<reference tests>>,gi;
	$result =~ s,([a-z]:)?\Q$unixTestCasePath\E(/)?,,gi;

	#print "testCasePath = $testCasePath\n";
	#print "unixTestCasePath = $unixTestCasePath\n";
	#print "scriptData = $pluginData\n";
	#print "DOSStyle_scriptData = $DOSStyle_scriptData\n";

	# print "\t4.5: $result";
	$result =~ s,([a-z]:)?\Q$pluginData\E(/)?,,gi;
	$result =~ s,([a-z]:)?\Q$DOSStyle_scriptData\E(/)?,,gi;

	# print "\t5: $result";
	if ( defined($testCasePattern) ) {
		$result =~
		  s/([^\s]*(\/|\\))?\Q$testCasePattern\E/<<reference tests>>/gi;
	}
	$result =~
s/([^\s]*(\/|\\))?(_)*instructor(_)*test(s)?\.(h|cpp)/<<reference tests>>/gio;
	$result =~
	  s/([^\s]*(\/|\\))?runinstructortests\.cpp/<<reference tests>>/gio;
...

{
	...
	if ( defined $localFiles and $localFiles ne "" ) {
...
my $objDir = catfile( $pluginHome, "obj" );
mkdir($objDir) if ( not -d $objDir );
...
if ($callAnt) {
	if ( $debug > 2 ) { $ANT .= " -v"; }
	my $buildXmlPath = catfile( $pluginHome, "build.xml" );
	my $cmdline =
	    $Web_CAT::Utilities::SHELL
	  . "$ANT -f \"$buildXmlPath\" -l \"$antLog\" "
	...
	if ($useSpawn) {
		...
		my ( $exitcode, $timeout_status ) =
		  Proc::Background::timeout_system( $timeout - $postProcessingTime,
			$cmdline );
		...
		if ($timeout_status) {
			$can_proceed = 0;
			my $feedbackGenerator = new Web_CAT::FeedbackGenerator($timeoutLog);
			$feedbackGenerator->startFeedbackSection("Errors During Testing");
...
sub scanTo {
	...
	if ( defined $_ and not m/$pattern/ ) {
		while (<ANTLOG>) {
			last if m/$pattern/;
		}
	}
...
sub adminLog {
	open SCRIPTLOG, ">>$scriptLog"
	  or die "Cannot open file for output '$scriptLog': $!";
	print SCRIPTLOG join( "\n", @_ ), "\n";
	close(SCRIPTLOG);
}

open ANTLOG, "$antLog" or die "Cannot open file for input '$antLog': $!";
$antLogOpened++;
$_ = <ANTLOG>;

if ( not $onlyInstructorTestsAllowed ) {
  ...
  #=============================================================================
	if ($can_proceed) {
		...
		$_ = <ANTLOG>;
		if ( defined $_
			and m/^\s*\[apply\].*(usage|no tests defined)/io )
		{
			$can_proceed = 0;
			open TESTLOG, ">$testLog"
			  or die "Cannot open file for output '$testLog': $!";
			print TESTLOG<<EOF;
...
EOF
		...
		print "cxxtest student generation analysis skipped\n";
	}
  ...
  #=============================================================================
	if ($can_proceed) {
		...
		if ( not defined $_ or $_ !~ m/^\s*\[cc\].*files to be compiled/ ) {
		...
		$_ = <ANTLOG>;
		while ( defined $_ and ( s/^\s*\[cc\] //o or m/^\s*$/o ) ) {
			if (m/^\s*$/o) { $_ = <ANTLOG>; next; }

			# print "msg: $_";
			if (   m/^(\s*[A-Za-z]:)?[^:]+:([0-9]*:)?\s*error:/o
				or m/no such file/io
				or m/ld returned 1 exit status/o )
			{
		...
		if ( $compileMsgs ne "" ) {
			open COMPILELOG, ">$compileLog"
			  or die "Cannot open file for output '$compileLog': $!";
...
EOF
  ...
  #=============================================================================
	if ($can_proceed) {
		...
		scanTo(qr/^\s*\[exec\]/);
		if ( not defined $_ or $_ !~ m/^\s*\[exec\]\s+/ ) {

			# adminLog( "Failed to find [junit] in line:\n$_" );
		}
		my $testMsgs        = "";
		my $timeoutOccurred = 0;
		while ( defined $_ and ( s/^\s*\[exec\] //o or m/^$/o ) ) {
...
"\n<p><font color=\"#ee00bb\">Testing your solution exceeded the allowable time limit for this assignment.</font></p>
	...
	\n";
		...
		if ( $testsFailed and $allStudentTestsMustPass ) {
		...
		if ( $testsRun == 0 or $testsFailed > 0 ) {
...
"\n<font color=\"#ee00bb\">No tests included in submission.</font>";
		...
		$studentCasesPercent =
		  $testsRun > 0
		  ? int( ( ( $testsRun - $testsFailed ) / $testsRun ) * 100.0 + 0.5 )
		  : 0;
		if ( $testsFailed and $studentCasesPercent == 100 ) {
		...
		if ( -f "memwatch.log" ) {
			open MEMWATCHLOG, "memwatch.log"
			  or die "Cannot open file for input 'memwatch.log': $!";
		...
		print "student test results analysis skipped\n";
	}
	my $time4 = time;
	print "\n", ( $time4 - $time3 ), " seconds\n" if ($debug);
...
if ($can_proceed) {
	...
	while ( defined $_ and ( s/^\s*\[cc\] //o or m/^\s*$/o ) ) {
		...
		if ( m/in (file|member)/io or m/\(Each/o ) {
			do {
				print "skipping: $_" if ( $debug > 4 );
				$_ = <ANTLOG>;
			} while ( defined $_ and m/^\s*\[cc\]\s\s+/o );
			next;
		}
		if ($collectingMsgs) {
			$compileMsgs .= prep_for_output($_);
...
if ($can_proceed) {
	...
	while ( defined $_ and ( s/^\s*\[exec\] //o or m/^\s*$/o ) ) {
	...
	if ( not $resultsSeen and $instructorTestsRun > 0 ) {
		$instructorTestsFailed = $instructorTestsRun;
		print "no results seen, failed = $instructorTestsFailed\n"
		  if ( $debug > 1 );
	}
	my $instrHints = "";
	if ( %instrHints or $memwatchLog ne "" ) {
		my $wantHints = $hintsLimit;
		if ($wantHints) {
			$instrHints =
			    "<p>The following hint(s) may help you locate "
			...
			foreach my $msg ( keys %instrHints ) {
				if ( $msg =~ m/^(assert|SIG)/o ) {
					...
					$instrHints .=
					  "hint: your code/tests do not correctly cover " . $msg;
					$hintsLimit--;
				}
			}
...
EOF
		...
		and $instructorTestsFailed == $instructorTestsRun )
	{
		$feedbackGenerator->print( <<EOF );
<p><b>Problem coverage: <font color="#ee00bb">unknown</font></b></p><p>
...
EOF
	...
	if ($can_proceed) {
		...
		if ( defined $_ and m/^BUILD FAILED/ ) {
	...
	if ( not $onlyInstructorTestsAllowed ) {
		$status{'studentTestResults'} =
		  new Web_CAT::JUnitResultsReader( catfile( $log_dir, "student.inc" ) );

		$status{'studentDerefereeStats'} = new Web_CAT::DerefereeStatsReader(
			catfile( $log_dir, "student-dereferee.inc" ) );
	}
	$status{'instrTestResults'} =
	  new Web_CAT::JUnitResultsReader( catfile( $log_dir, "instr.inc" ) );
	$status{'instrDerefereeStats'} = new Web_CAT::DerefereeStatsReader(
		catfile( $log_dir, "instr-dereferee.inc" ) );
}
elsif ($debug) {
	print "instructor test results analysis skipped\n";
}

if ($antLogOpened) {
	close(ANTLOG);
	my $headerFile = catfile( $log_dir, "ant.header" );
...
if ($can_proceed) {
	...
	if ( not $onlyInstructorTestsAllowed and $testsFailed ) {
...
sub extractEntity {
	...
	if ( $line =~ s/^\s*case\b\s*//o ) {
		...
		if ( length($line) ) {
			...
			$last = substr( $line, -1 );
			if (
				( $first ne "'" and $first ne '"' and $first ne "(" )
				or (    $last ne "'"
					and $last ne '"'
					and $last ne ")" )
			  )
...
sub exclude_from_coverage {
	...
	if (
		path_contains_part_of_path( $fqFileName, $pluginData )
		or ( defined($assignmentIncludes)
			and path_contains_part_of_path( $fqFileName, $assignmentIncludes ) )
		or ( defined($generalIncludes)
			and path_contains_part_of_path( $fqFileName, $generalIncludes ) )
	  )
	{
		return 1;
	}
...
sub strip_quotes_and_normalize {
	...
	if ( length($fqFileName) > 0
		and substr( $fqFileName, 0, 1 ) eq '"' )
	{
		$fqFileName = substr( $fqFileName, 1 );
	}
	if ( length($fqFileName) > 0
		and substr( $fqFileName, -1 ) eq '"' )
	{
...
if ( $testsRun and $measureCodeCoverage ) {
	my $coverageLog = catfile( $log_dir, "covfile.out" );
	...
	if ( open( COVERAGE, $coverageLog ) ) {
		...
		if (    $topLevelGradedElements > 0
			and $runtimeScoreWithoutCoverage > 0 )
		{
			$ptsPerUncovered =
			  -1.0 / $topLevelGradedElements * $runtimeScoreWithoutCoverage;
		}
		...
		if ( open( COVERAGE, $coverageLog ) ) {
			...
			if ( $gradedElements > 0 ) {
				$codeCoveragePercent =
				  $gradedElements > 0
				  ? int(
					( $gradedElementsCovered / $gradedElements ) * 100.0 + 0.5 )
				  : 0;
				if (    $gradedElementsCovered < $gradedElements
					and $codeCoveragePercent == 100 )
				{
		...
		$coverageLog = catfile( $log_dir, "covfunc.out" );
		if ( open( COVERAGE, $coverageLog ) ) {
			while (<COVERAGE>) {
				chomp;
				my @entry = split(/,/);
				...
				if ( $entry[3] eq "function" and $entry[4] eq "" ) {
					if ( not defined $codeMessages{ $entry[0] } ) {
						$codeMessages{ $entry[0] } = {};
					}
					if ( not defined $codeMessages{ $entry[0] }{ $entry[1] } ) {
		...
		# Now process the CSV file for source info
		$coverageLog = catfile( $log_dir, "covsrc.out" );
		if ( open( COVERAGE, $coverageLog ) ) {
			my $thisFile;
			while (<COVERAGE>) {
				chomp;
				...
				if (s/^\s*-->//o) {
					...
					if ( not defined $codeMessages{$thisFile} ) {
						$codeMessages{$thisFile} = {};
					}
					if ( not defined $codeMessages{$thisFile}{$line} ) {
...
EOF
		...
		if ( $coverageMetric and $gradedElements > 0 ) {
...
if ( not $onlyInstructorTestsAllowed ) {
	if ( defined $status{'studentTestResults'}
		and $status{'studentTestResults'}->hasResults )
	{
	...
	if ( defined $status{'studentDerefereeStats'}
		and $status{'studentDerefereeStats'}->hasResults )
	{
...
if ( defined $status{'instrTestResults'}
	and $status{'instrTestResults'}->hasResults )
{
...
if ( defined $status{'instrDerefereeStats'}
	and $status{'instrDerefereeStats'}->hasResults )
{
	...
	$cfg->setProperty(
		'instructor.memory.numCallsToArrayDelete',
...
my $outcomeProperties =
    "(\"instructor.test.results\""
  . ( ( not $onlyInstructorTestsAllowed ) ? ", \"student.test.results\"" : "" )
  . ")";

$cfg->setProperty( 'outcomeProperties', $outcomeProperties );
...
if ($can_proceed) {
	...
	if (   $instructorTestsRun == 0
		or $instructorTestsFailed == $instructorTestsRun )
	{
...
<p>score = $maxCorrectnessScore 
EOF
	$feedbackGenerator->print(" * $studentCasesPercent%")
	  if ( not $onlyInstructorTestsAllowed );
	$feedbackGenerator->print(" * $codeCoveragePercent% ") if ($coverageMetric);
...
$beautifier->beautifyCwd( $cfg, ['runAllTests.cpp'], \%codeMarkupIds,
	\%codeMessages );
